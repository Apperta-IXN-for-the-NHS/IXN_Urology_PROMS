...
export function usePhotoGallery(storageKey: string) {
  const [photos, setPhotos] = useState<Photo[]>([]);
  const PHOTO_STORAGE_KEY = storageKey;
  ...
  useEffect(() => {
    const loadSaved = async () => {
      const photosString = await get(PHOTO_STORAGE_KEY);
  ...
  const deletePhoto = async (photo: Photo) => {
    ...
    set(PHOTO_STORAGE_KEY, JSON.stringify(newPhotos));
